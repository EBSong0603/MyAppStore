...
class DetailSixthViewFirst: ModuleView {
    ...
    private let reviewLabel: UILabel = {
       let label = UILabel()
        label.setStyle("평가 및 리뷰", textColor: .black, font: UIFont.systemFont(ofSize: 15), textAlignment: .left)
        return label
    }()
    
    private let furtherLabel: UILabel = {
      let label = UILabel()
        label.setStyle("모두보기", textColor: .black, font: UIFont.systemFont(ofSize: 15), textAlignment: .left)
        return label
    }()
